      ...
      <div className="relative h-[60vh] md:h-[80vh] overflow-hidden">
        ...
        <div className="absolute inset-x-0 bottom-0 p-4 md:p-12 max-w-4xl">
          <FadeIn>
            ...
            <p className="mt-2 text-sm text-amber-gold tracking-wider">
        ...
        {product.features.map((feature, index) => (
          ...
          >
            <div className={index % 2 === 1 ? "md:[direction:ltr]" : ""}>
              <p className="text-xs uppercase tracking-[0.25em] text-amber-gold mb-3">
              ...
              {feature.id === "feature-suction" && (
                <div className="mt-4 inline-block rounded-lg border border-amber-gold/20 bg-amber-gold/5 px-4 py-2">
  ...
  const visuals: Record<string, ReactNode> = {
    "feature-reach": (
      <div className="absolute inset-0 flex items-center justify-center">
        <div className="relative w-32 h-32 rounded-full border-2 border-charcoal bg-charcoal/80">
          <div className="absolute -right-4 top-1/2 -translate-y-1/2 w-8 h-1 bg-amber-gold/60 rounded animate-extend-brush" />
          <div className="absolute inset-4 rounded-full border border-amber-gold/30 animate-pulse-slow" />
        </div>
        <div className="absolute bottom-8 right-8 w-16 h-16 border border-amber-gold/20 rounded-lg" />
      </div>
    ),
    "feature-suction": (
      <div className="absolute inset-0 flex items-end justify-center pb-8">
        <div className="flex gap-1 items-end h-32">
          {Array.from({ length: 12 }).map((_, i) => (
            <div
              key={i}
              className="w-2 bg-gradient-to-t from-amber-gold/20 to-warm-white/10 rounded-t"
    ...
    "feature-vision": (
      <div className="absolute inset-0 bg-navy-deep">
        <div className="absolute inset-8 border border-amber-gold/20 rounded-lg">
          <div className="absolute top-1/3 left-1/4 w-8 h-8 rounded-full border border-amber-gold/40 animate-pulse-slow" />
          <div className="absolute bottom-1/3 right-1/3 w-12 h-6 rounded border border-amber-gold/30" />
    ...
    "feature-real-life": (
      <div className="absolute inset-0 flex items-center justify-center">
        ...
        <div className="absolute bottom-12 left-1/2 -translate-x-1/2 w-24 h-24 rounded-full border-2 border-charcoal bg-charcoal/80">
          <div className="absolute inset-0 rounded-full bg-amber-gold/5 animate-pulse-slow" />
        </div>
      </div>
    ),
    "feature-self-clean": (
      <div className="absolute inset-0 flex items-end justify-center pb-4">
        <div className="relative w-20 h-28 bg-charcoal/60 rounded-t-lg border border-warm-white/10">
          ...
          <div className="absolute bottom-4 inset-x-2 h-1 bg-amber-gold/30 rounded animate-pulse-slow" />
        </div>
      </div>
    ),
  };
